...
import { editCommentPaymentThunk } from '../../store/payments';
import '../EditExpenseComment/EditExpenseComment.css'


function EditPaymentCommentForm({ payment_id, comment_id, original_text, toggleEditComment }) {
    ...
    const handleSubmitEdit = async (e) => {
        ...
        try {
            const data = await dispatch(editCommentPaymentThunk(payment_id, comment_id, comment))
            if (data.error) {
    ...
    return (
        <form className='edit_comment_form flex_col display_none' onSubmit={handleSubmitEdit} id={`payment_comment${comment_id}`}>
...
export default EditPaymentCommentForm;
