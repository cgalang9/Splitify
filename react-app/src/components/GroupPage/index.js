...
const GroupPage = () => {
  ...
  useEffect(() => {
    const payment_detail_sections = document.querySelectorAll(
      ".activity_payment_details"
    );
    payment_detail_sections.forEach((section) => {
      section.classList.remove("active_details");
    });
    const expense_detail_sections = document.querySelectorAll(
              ...
              group_members.members.map((member) => (
                <div key={member.user_id} id="group_page_right_member_item">
                  <img
                    src={user_icon_img}
                    alt="user_icon"
                    className="user_icon_group_right"
                  />
                      ...
                      <>
                        ...
                        {balances[member.user_id] > 0 && (
                          <div style={{ color: "green" }}>
                            gets back ${balances[member.user_id].toFixed(2)}
                          </div>
                        )}
                        {balances[member.user_id] < 0 && (
                          <div style={{ color: "red" }}>
                            owes ${(balances[member.user_id] * -1).toFixed(2)}
                          </div>
                        )}
                      </>
                    )}
