...
import {
  getCurrGroupMembersThunk,
  clearGroupMembers,
} from "../../store/currentGroupMembers";
...
function AddExpenseForm({ closeModal }) {
  ...
  useEffect(async () => {
    ...
    //clear checkboxes and reset payer when changing group
    setCheckedUsers([]);
  ...
  const handleSubmit = async (e) => {
    ...
    let total_per_person = parseFloat(
      total / (checkedUsers.length + 1)
    ).toFixed(2);
    ...
    <div className="expense_form_wrapper flex_col">
      <form className="expense_form flex_col" onSubmit={handleSubmit}>
        <div className="expense_form_head">
          ...
          <div className="expense_form_x" onClick={closeModal}>
        ...
        <div className="expense_form_users">
          <div htmlFor="split" className="expense_form_users_title">
            With you and:{" "}
          </div>
        ...
        <div className="expense_form_main_inputs">
          <img
            src={cat_icon_img}
            alt="category_icon"
            className="expense_form_cat_icon"
          />
          <div className="expense_form_main_inputs_right flex_col">
            ...
            <div className="expense_form_total_container">
              ...
              <input
                ...
                required
                min={1.0}
                placeholder={parseFloat(0.0).toFixed(2)}
              />
            </div>
          </div>
        </div>
        ...
        <div className="expense_form_btn_container">
          <button
            type="button"
            className="expense_form_cancel"
            onClick={closeModal}
          >
            Cancel
          </button>
          <button type="submit" className="expense_form_save">
            Save
          </button>
        </div>
      </form>
