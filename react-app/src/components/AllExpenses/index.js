...
const AllExpenses = () => {
  ...
  useEffect(async () => {
    ...
    await setSortedActivity(
      expenses_payments.sort(
        (a, b) =>
      ...
      {isLoaded && (
        <div id="all_wrapper">
          ...
          <div id="all_mid" className="flex_col">
            <div id="all_head">
              <div id="all_head_title">All Expenses</div>
              ...
              {sortedActivity.length > 0 &&
                sortedActivity.map((activity, idx) => (
                  <div key={idx} className="all_activity_li">
                    ...
                    {activity.payee && <PaymentListItem activity={activity} />}
                  </div>
                ))}
            </div>
          </div>
          <div id="all_right">
            ...
            {balance === 0 && <div>You are settled up</div>}
            {balance > 0 && (
              <>
                <div style={{ color: "green" }} className="all_right_top_total">
                  you are owed
                </div>
                <div style={{ color: "green" }} className="all_right_total">
                  ${balance.toFixed(2)}
                </div>
              </>
            )}
            {balance < 0 && (
              <>
                <div style={{ color: "red" }} className="all_right_top_total">
                  you owe
                </div>
                <div style={{ color: "red" }} className="all_right_total">
                  ${(balance * -1).toFixed(2)}
                </div>
              </>
            )}
            <div id="all_right_links_title">MY LINKS</div>
            <div id="all_right_github_container">
              <a
                target="_blank"
                rel="noopener noreferrer"
                href="https://github.com/cgalang9"
              >
                <img src={github} alt="github_logo" id="all_right_github" />
              </a>
            </div>
